...
[[ "${__usage+x}" ]] || read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -d --directory  [arg] Directory to process.
...
[[ "${__helptext+x}" ]] || read -r -d '' __helptext <<-'EOF' || true # exits non-zero when EOF encountered
 This script run against a folder will look for duplicate files and create hardlinks
 to a files in a hidden folder. Further duplicated will be deleted and replaced with a hardlink.
EOF
